 ...
 */
	protected function _parseKey(Model $Model, $key, $value) {
		...
		if ($key === 'text_phrase') {
			return array('query' => array('text_phrase' => $value));
		}

		if ($key === 'fuzzy') {
			return array('query' => array('fuzzy' => $value));
		}

		if ($key === 'common') {
			return array('query' => array('common' => $value));
		}
 ...
 **/
	public function geoDistance($field, array $options) {
		...
		$opts += array('unit' => 'km', 'distance_type' => 'arc');
		return array(
			'geo_distance' => array(
					...
					foreach ($types as $type => $models) {
						foreach ($models['properties'] as $alias => $properties) {
							if (strpos($alias, '.') > 0) {
								$alias = explode('.', $alias);
								if (!empty($properties)) {
									$schema[$alias[0]][$alias[1]] = $properties;
								}
							} elseif (!empty($properties['properties'])) {
