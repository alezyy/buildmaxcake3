...
class SearchableBehavior extends ModelBehavior {
	...
	public function afterDelete (Model $Model) {
		...
		// Index needs a moment to be updated
		$res = $this->execute($Model, 'DELETE', $Model->id);
	...
	public function fill () {
		...
		// Create index
		$u = $this->execute($Model, 'PUT', '', array(), array('fullIndex' => true));
		$d = $this->execute($Model, 'DELETE', '');
		$o = $this->execute($Model, 'POST', '_refresh', array(), array('fullIndex' => true));

		$index_name = $this->opt($Model, 'index_name');
		$type       = $this->opt($Model, 'type');
		$map = $this->execute($Model, 'PUT', '_mapping', array(
			$type => array(
				'properties' => array(
					$Model->name  => array(
						'properties' => array(
...
//        prd(compact('logs'));
		...
		foreach ($results as $result) {
			...
			if (empty($doc[$Model->alias][$Model->primaryKey])) {
				return $this->err(
					$Model,
			...
			$meta = array(
				'_index' => $index_name,
				'_type' => $type,
				'_id' => $doc[$Model->alias][$Model->primaryKey],
			);

			//$doc['_id'] = $doc[$Model->alias][$Model->primaryKey];
			if (!($meta['_id'] % 100)) {
				$this->progress($Model, '(compile: @' . $meta['_id'] . ')');
			}

			$doc['_label'] = '';
			if (array_key_exists($Model->displayField, $doc[$Model->alias])) {
				$doc['_label'] = $doc[$Model->alias][$Model->displayField];
			}

			// FakeFields
			if (is_array($fake_fields)) {
				foreach ($fake_fields as $fake_field => $xPaths) {
					...
					$doc[$fake_field] = implode($join, $concats);

				}
			}
			$doc['_descr'] = '';
			if ($descKeyPath && array_key_exists(@$Model->descripField, $doc[$Model->alias])) {
				$doc['_descr'] = $doc[$Model->alias][@$Model->descripField];
			}
			...
			// $clean = function ($to_clean) use (&$clean, &$Model) {
			// 	if (!$Model->Behaviors->loaded('Diacritics')) {
			// 		$Model->Behaviors->load('Diacritics');
			// 	}
			// 	$out = array();
			...
			// 				$out[$key] = $Model->remove_accents($value);
			// 			}
			// 		}
			// 	} elseif (is_string($to_clean)) {
			// 		return $Model->remove_accents($to_clean);
			// 	}
			// 	return $out;
			// };
...
//            }
	...
	public function setup (Model $Model, $settings = array()) {
		...
		$this->settings[$Model->alias]['host'] = $DB['index']['host'];
		$this->settings[$Model->alias]['port'] = $DB['index']['port'];

		$this->settings[$Model->alias]['index_name'] = $DB['index']['index'];
