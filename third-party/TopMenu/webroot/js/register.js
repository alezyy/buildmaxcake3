$(document).ready(function (){
	...
	$('#ProfilePostalCode1').typeahead({
		source: postal_code_one,
		items: 8
	});

	$('#ProfilePostalCode2').typeahead({
		items: 8
	});

	$('#ProfileStreet').typeahead({
		items: 8
	});

	$('#ProfilePostalCode1').keyup(function () {
		var value = $(this).val();
		if (value.length >= 3) {
			$('#ProfilePostalCode2').focus();
		}
	});
	$('#ProfilePostalCode1').blur(function () {
		if ($(this).val().length >= 3) {
			var value = $(this).val();
			$.get('/street_addresses/getPostalCodesPart2/' + value + '.json', function (data) {
				var autocomplete = $('#ProfilePostalCode2').typeahead();
				autocomplete.data('typeahead').source = data.codes;
			});
		}
	});
	$('#ProfilePostalCode2').keyup(function () {
		var part_two = $(this).val();
		if (part_two.length >= 3) {
			$('#ProfileStreetNumber').focus();
		}
	});
	$('#ProfilePostalCode2').blur(function () {
		var part_one = $('#ProfilePostalCode1').val();
		var part_two = $(this).val();
		if (part_one.length >= 3 && part_two.length >= 3) {
			$.get('/street_addresses/getStreets/' + part_one + ' ' + part_two + '.json', function (data) {
				var autocomplete = $('#ProfileStreet').typeahead();
				autocomplete.data('typeahead').source = data.street_names;
			});
		}
	})
});