 ...
 */
	public function process($field, $sizes = array()) {
		if (strpos($field, '.')) {
			$field = explode('.', $field);
		...
		if (!empty($this->request->data[$model_name][$field]['tmp_name'])) {
			...
			$new_name = $this->processImage($file['tmp_name'], $old_image);
 ...
 */
	public function processImage($image, $old_image = null, $prefix = null) {
		...
		foreach ($this->sizes as $dimention) {
			if ($dimention !== 'original') {
				$dimentions = explode('x', $dimention);

				if (sizeof($dimentions) < 2) {
					return false;
				}
			...
			$extention           = '.jpg';
			$new_name_dimentions = $prefix . $new_name . '_' . $dimention . $extention;
			$old_image_name      = $prefix . $old_image . '_' . $dimention . $extention;
			

			$this->resize_image($original_image, $x, $y, $new_name_dimentions);
			$this->convertTojpeg($new_name_dimentions, $new_name_dimentions);
 ...
 * @param  int    $new_x              New X dimention
 * @param  int    $new_y              New Y dimention
 * @return void
 */
	public function resizeImage($image, $original_extension , $new_x, $new_y) {
		...
		$extention           = '.jpg';
		$new_name_dimentions = $prefix . $image . '_' . $new_x . 'x' . $new_y . '.' . $extention;


		$this->resize_image($original_image, $new_x, $new_y, $new_name_dimentions);
		$this->convertTojpeg($new_name_dimentions, $new_name_dimentions);
 ...
 * @param  mixed  $image
 * @param  integer $x     x-dimention
 * @param  integer $y     y-dimention
 * @return void
 */
	private function resize_image($image, $x = 64, $y = 64, $new_image = null) {
        ...
        exec("convert -filter Lanczos -resize {$x}x{$y} -quality 85% -strip -unsharp 0.8x0.8+0.3+0.008 -units PixelsPerInch -density 72 '$original_image' '$new_image'");
	}
 ...
 */
	private function generate_name($prefix = null, $extention = '.jpg') {
		...
		while(1) {
			$string = String::uuid();
			$found = false;
			foreach ($this->sizes as $dimentions) {
				if (file_exists($filename . $string . '_' . $dimentions . $extention)) {
					$found = true;
					break;
				}
			}
