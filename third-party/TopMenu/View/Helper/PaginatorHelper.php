 ...
 */
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if ($this->{$check}($model)) {
			...
			unset($options['rel']);
			return $this->Html->tag($tag, $this->link($title, null, array_merge($options, compact('escape'))), compact('class'));
		}
	}
 ...
 */
	public function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			$out .= $this->Html->tag($tag, $this->link($params['page'], array('page' => $params['page'])), array('class' => $currentClass));
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					...
					$out .= $this->Html->tag($tag, $this->link($i, array()), array('class' => $currentClass));
				} else {
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options), compact('class'));
				}
				if ($i != $params['pageCount']) {
 ...
 */
	public function first($first = '<< first', $options = array()) {
		...
		if (is_int($first) && $params['page'] >= $first) {
			if ($after === null) {
				$after = $this->Html->tag($tag, $this->link($ellipsis, array()), array('class' => 'disabled'));
			}
			for ($i = 1; $i <= $first; $i++) {
				$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options), compact('class'));
				if ($i != $first) {
 ...
 */
	public function last($last = 'last >>', $options = array()) {
		...
		if (is_int($last) && $params['page'] <= $lower) {
			if ($before === null) {
				$before = $this->Html->tag($tag, $this->link($ellipsis, array()), array('class' => 'disabled'));
			}
