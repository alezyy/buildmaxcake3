 ...
 */
	public function _introspectModel($model, $key, $field = null) {
 ...
 */
	public function create($model = null, $options = array()) {
		...
		if ($options['url_append'] !== null) {
			$action .= $options['url_append'];
		}
 ...
 */
	public function input($fieldName, $options = array()) {
		...
		if (!empty($div)) {
			if (!isset($options['control_group']) || $options['control_group']) {
				$divOptions['class'] = 'controls';
			}
		...
		switch ($type) {
			...
			$wrapDivOptions = array('class' => 'control-group');


		if ($type != 'hidden' && $error !== false) {
			$errMsg = $this->error($fieldName, $error);
			if ($errMsg) {
				$wrapDivOptions = $this->addClass($wrapDivOptions, 'error');
				$out['error'] = $errMsg;
			} elseif ($type != 'hidden' && $error !== false && is_string($error)) {
				$wrapDivOptions = $this->addClass($wrapDivOptions, 'error');
				$out['error'] = $this->Html->tag(
					'span',
					$error,
					array(
						'class' => 'help-inline'
		...
		$output = $this->Html->tag('div', $output, $wrapDivOptions);
		return $output;
	}
 ...
 */
	public function radio($fieldName, $options = array(), $attributes = array()) {
		...
		foreach ($options as $optValue => $optTitle) {
			...
			$optionsHere['style'] = 'margin-right:10px';
 ...
 */
	public function day($fieldName = null, $attributes = array()) {
		$attributes += array('empty' => true, 'value' => null, 'class' => 'span1');
